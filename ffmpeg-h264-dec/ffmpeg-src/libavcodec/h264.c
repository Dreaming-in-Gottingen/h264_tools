...
int h264_get_dimension(AVCodecContext *avctx, int *aligned_width, int *aligned_height, int *crop_width, int *crop_height)
{
    H264Context *h = avctx->priv_data;

    if (h == NULL) {
        av_log(avctx, AV_LOG_ERROR, "avctx->priv_data==NULL\n");
        return -1;
    }

    if (aligned_width && aligned_height && crop_width && crop_height) {
        *aligned_width = h->sps.mb_width * 16;
        *aligned_height = h->sps.mb_height * 16;
        *crop_width = h->sps.mb_width * 16 - (h->sps.crop_right + h->sps.crop_left);
        *crop_height = h->sps.mb_height * 16 - (h->sps.crop_bottom + h->sps.crop_top);
        return 0;
    } else {
        return -1;
    }
...
AVCodec ff_h264_decoder = {
    ...
    .get_video_dimension   = h264_get_dimension,
};
